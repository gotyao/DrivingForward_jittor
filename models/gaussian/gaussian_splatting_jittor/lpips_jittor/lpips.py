...
import models.gaussian.gaussian_splatting_jittor.lpips_jittor.pretrained_networks as pn
        ...
        if(lpips):
            ...
            if(pretrained):
                if(model_path is None):
                    #import inspect
                    #import os
                    #model_path = os.path.abspath(os.path.join(inspect.getfile(self.__init__), 'PerceptualSimilarity-master/lpips', 'weights/v%s/%s.pth'%(version,net)))
                    model_path = 'jt_lpips_%s_v01.pkl'%net
                if(verbose):
                    print('Loading model from: %s'%model_path)
                model_dict = jt.load(model_path)
                self.load_state_dict(model_dict)
                #import torch
                #weight_dict = torch.load('../PerceptualSimilarity-master/lpips/weights/v0.1/squeeze.pth')
                #self.load_state_dict(weight_dict)
                #jt.save(self.state_dict(), './lpips/weights/jt_lpips_squeeze_v01.pkl')
