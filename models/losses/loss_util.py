...
def compute_auto_masks(reprojection_loss, identity_reprojection_loss):
    ...
    if identity_reprojection_loss is None:
        ...
        reprojection_loss_mask = jt.ones_like(reprojection_loss)
    else:
        # using auto(identity loss) mask
        losses = jt.contrib.concat([reprojection_loss, identity_reprojection_loss], dim=1)
        idxs, _ = jt.argmin(losses, dim=1, keepdims=True)
...
def compute_ssim_loss(pred, target):
    ...
    """
    ref_pad = jt.nn.ReflectionPad2d(1)
    pred = ref_pad(pred)
    target = ref_pad(target)

    mu_pred = jt.nn.avg_pool2d(pred, kernel_size = 3, stride = 1)
    mu_target = jt.nn.avg_pool2d(target, kernel_size = 3, stride = 1)

    musq_pred = mu_pred.pow(2)
    musq_target = mu_target.pow(2)
    mu_pred_target = mu_pred*mu_target

    sigma_pred = jt.nn.avg_pool2d(pred.pow(2), kernel_size = 3, stride = 1)-musq_pred
    sigma_target = jt.nn.avg_pool2d(target.pow(2), kernel_size = 3, stride = 1)-musq_target
    sigma_pred_target = jt.nn.avg_pool2d(pred*target, kernel_size = 3, stride = 1)-mu_pred_target
    ...
    return jt.clamp((1-ssim_map)/2, 0, 1)
