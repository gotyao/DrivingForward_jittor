...
class SingleCamLoss(BaseLoss):
    ...
    def compute_reproj_loss(self, inputs, target_view, cam=0, scale=0, ref_mask=None):
        ...
        reprojection_losses = jt.contrib.concat(reprojection_losses, 1)
        reprojection_loss = jt.min(reprojection_losses, dim=1, keepdims=True)
        ...
        identity_reprojection_losses = jt.contrib.concat(identity_reprojection_losses, 1)
        identity_reprojection_losses = identity_reprojection_losses + \
                                        _EPSILON * jt.randn_like(identity_reprojection_losses)
        identity_reprojection_loss = jt.min(identity_reprojection_losses, dim=1, keepdims=True)
