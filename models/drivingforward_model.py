...
class DrivingForwardModel(BaseModel):
    ...
    def set_posenet(self, cfg):
        return PoseNetwork(cfg)
        
    def set_depthnet(self, cfg):
        return DepthNetwork(cfg)

    def set_gaussiannet(self, cfg):
        return GaussianNetwork(rgb_dim=3, depth_dim=1)
    ...
    def set_train_dataloader(self, cfg, rank):                 
        ...
        self._dataloaders['train'] = train_dataset.set_attrs(batch_size=self.batch_size, shuffle=True, num_workers=0, drop_last=True)
    ...
    def set_val_dataloader(self, cfg):         
        ...
        self._dataloaders['val'] = val_dataset.set_attrs(batch_size=self.batch_size, shuffle=False, num_workers=0, drop_last=True)
    ...
    def set_eval_dataloader(self, cfg):  
        ...
        self._dataloaders['eval'] = eval_dataset.set_attrs(batch_size=self.eval_batch_size, shuffle=False, num_workers=0, drop_last=True)
    ...
    def estimate(self, inputs):
        ...
        inputs['extrinsics_inv'] = jt.linalg.inv(inputs['extrinsics'])
    ...
    def to_depth(self, disp_in, K_in):        
        ...
        disp_in = jt.nn.interpolate(disp_in, [self.height, self.width], mode='bilinear', align_corners=False)
        ...
        if self.novel_view_mode == 'MF':
            for frame_id in self.frame_ids:
                if frame_id == 0:
                    ...
                    for i in range(bs):
                        ...
                        projection_matrix = getProjectionMatrix(znear=znear, zfar=zfar, K=intr, h=height, w=width).transpose(0, 1)
                        world_view_transform = jt.Var(extr).transpose(0, 1)
                        # full_proj_transform: (E^T K^T) = (K E)^T
                        full_proj_transform = jt.nn.bmm(world_view_transform.unsqueeze(0), (projection_matrix.unsqueeze(0))).squeeze(0)
                        camera_center = jt.linalg.inv(world_view_transform)[3, :3]
                    ...
                    outputs[('cam', cam)][('FovX', frame_id, 0)] = jt.Var(FovX_list)
                    outputs[('cam', cam)][('FovY', frame_id, 0)] = jt.Var(FovY_list)
                    outputs[('cam', cam)][('world_view_transform', frame_id, 0)] = jt.contrib.concat(world_view_transform_list, dim=0)
                    outputs[('cam', cam)][('full_proj_transform', frame_id, 0)] = jt.contrib.concat(full_proj_transform_list, dim=0)
                    outputs[('cam', cam)][('camera_center', frame_id, 0)] = jt.contrib.concat(camera_center_list, dim=0)
                else:
                    outputs[('cam', cam)][('e2c_extr', frame_id, 0)] = \
                        jt.matmul(outputs[('cam', cam)][('cam_T_cam', 0, frame_id)], inputs['extrinsics_inv'][:, cam, ...])
                    outputs[('cam', cam)][('c2e_extr', frame_id, 0)] = \
                        jt.matmul(inputs['extrinsics'][:, cam, ...], jt.linalg.inv(outputs[('cam', cam)][('cam_T_cam', 0, frame_id)]))
            ...
            for frame_id in self.frame_ids[1:]:
                ...
                for i in range(bs):
                    ...
                    FovY = focal2fov(intr[1, 1], height)
                    projection_matrix = getProjectionMatrix(znear=znear, zfar=zfar, K=intr, h=height, w=width).transpose(0, 1)
                    world_view_transform = jt.matmul(T_i, jt.Var(extr)).transpose(0, 1)
                    # full_proj_transform: (E^T K^T) = (K E)^T
                    full_proj_transform = jt.nn.bmm(world_view_transform.unsqueeze(0), (projection_matrix.unsqueeze(0))).squeeze(0)
                    camera_center = jt.linalg.inv(world_view_transform)[3, :3]
                    FovX_list.append(FovX)
