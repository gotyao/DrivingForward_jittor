...
class ViewRendering(nn.Module):
    ...
    def get_mean_std(self, feature, mask):
        ...
        """
        _, c, h, w = mask.size()
        mean = (feature * mask).sum((1,2,3), keepdim=True) / (mask.sum((1,2,3), keepdim=True) + 1e-8)
        var = ((feature - mean) ** 2).sum((1,2,3), keepdim=True) / (c*h*w)
        return mean, jt.sqrt(var + 1e-16)
    ...
    def get_virtual_image(self, src_img, src_mask, tar_depth, tar_invK, src_K, T, scale=0):
        ...
        img_warped = nn.grid_sample(src_img, pix_coords, mode='bilinear',
                                    padding_mode='zeros', align_corners=True)
        mask_warped = nn.grid_sample(src_mask, pix_coords, mode='nearest',
                                    padding_mode='zeros', align_corners=True)

        # nan handling
        inf_img_regions = jt.isnan(img_warped)
        img_warped[inf_img_regions] = 2.0
        inf_mask_regions = jt.isnan(mask_warped)
        mask_warped[inf_mask_regions] = 0

        pix_coords = pix_coords.permute(0, 3, 1, 2)
        invalid_mask = (pix_coords > 1).logical_or(pix_coords < -1).sum((1), keepdim=True) > 0
        return img_warped, (invalid_mask.logical_not()).float() * mask_warped
    ...
    def get_virtual_depth(self, src_depth, src_mask, src_invK, src_K, tar_depth, tar_invK, tar_K, T, min_depth, max_depth, scale=0):
        ...
        src_points_warped = jt.matmul(T[:, :3, :], src_points)
        ...
        pix_coords = self.project(tar_depth, jt.linalg.inv(T), tar_invK, src_K)
        depth_warped = nn.grid_sample(src_depth_warped, pix_coords, mode='bilinear',
                                        padding_mode='zeros', align_corners=True)
        mask_warped = nn.grid_sample(src_mask, pix_coords, mode='nearest',
                                    padding_mode='zeros', align_corners=True)

        # nan handling
        inf_depth = jt.isnan(depth_warped)
        depth_warped[inf_depth] = 2.0
        inf_regions = jt.isnan(mask_warped)
        ...
        return depth_warped, (invalid_mask.logical_not()).float() * mask_warped * valid_depth_min * valid_depth_max
        
    def execute(self, inputs, outputs, cam, rel_pose_dict):
        ...
        for scale in self.scales:           
            ...
            if self.spatio or self.spatio_temporal:
                for frame_id in self.frame_ids:
                    overlap_img = jt.zeros_like(ref_color)
                    overlap_mask = jt.zeros_like(ref_mask)
