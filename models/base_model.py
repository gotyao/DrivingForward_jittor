...
class BaseModel:
    ...
    def save_model(self, epoch):
        ...
        for model_name, model in self.models.items():
            model_file_path = os.path.join(curr_model_weights_dir, f'{model_name}.p')
            to_save = model.state_dict()
            jt.save(to_save, model_file_path)
        
        # save optimizer
        optim_file_path = os.path.join(curr_model_weights_dir, f'{_OPTIMIZER_NAME}.p')
        jt.save(self.optimizer.state_dict(), optim_file_path)
    ...
    def load_weights(self):
        ...
        for n in self.models_to_load:
            ...
            path = os.path.join(self.load_weights_dir, f'{n}.p')
        ...
        if self.mode == 'train':
            # loading adam state
            optim_file_path = os.path.join(self.load_weights_dir, f'{_OPTIMIZER_NAME}.p')
            if os.path.isfile(optim_file_path):
                try:
                    print(f'Loading {_OPTIMIZER_NAME} weights')
                    optimizer_dict = jt.load(optim_file_path)
