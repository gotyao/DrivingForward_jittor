...
from external.packnet_sfm.utils.misc import filter_dict
from external.packnet_sfm.utils.types import is_seq
...
def resize_image(image, shape, interpolation=Image.ANTIALIAS):
    ...
    image : Image.PIL
        Resized image
    """
    transform = transforms.Resize(shape, mode=interpolation)
...
def resize_sample_image_and_intrinsics(sample, shape,
    ...
    image_transform = transforms.Resize(shape, mode=image_interpolation)
...
def to_tensor(image, tensor_type='torch.FloatTensor'):
    ...
    return jt.float64(transform(image))
...
def to_tensor_sample(sample, tensor_type='torch.FloatTensor'):
    ...
    for key in filter_dict(sample, [
        ...
        sample[key] = jt.float64(transform(sample[key]))
    sample['depth'] = sample['depth'].permute(2, 0, 1)
    # Convert lists
    for key in filter_dict(sample, [
        'rgb_context', 'rgb_context_original', 'depth_context'
    ]):
        sample[key] = [jt.float64(transform(k)) for k in sample[key]]
...
def random_color_jitter_transform(parameters):
    ...
    if brightness is not None:
        ...
        all_transforms.append(transforms.Lambda(
            lambda img: transforms.adjust_brightness(img, brightness_factor)))
    # Add contrast transformation
    if contrast is not None:
        contrast_factor = random.uniform(contrast[0], contrast[1])
        all_transforms.append(transforms.Lambda(
            lambda img: transforms.adjust_contrast(img, contrast_factor)))
    # Add saturation transformation
    if saturation is not None:
        saturation_factor = random.uniform(saturation[0], saturation[1])
        all_transforms.append(transforms.Lambda(
            lambda img: transforms.adjust_saturation(img, saturation_factor)))
    # Add hue transformation
    if hue is not None:
        hue_factor = random.uniform(hue[0], hue[1])
        all_transforms.append(transforms.Lambda(
            lambda img: transforms.adjust_hue(img, hue_factor)))
