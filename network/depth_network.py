...
class DepthNetwork(nn.Module):
    ...
    def execute(self, inputs):
        ...
        sf_images = jt.stack([inputs[('color_aug', 0, 0)][:, cam, ...] for cam in range(self.num_cams)], 1)
        if self.novel_view_mode == 'MF':
            sf_images_last = jt.stack([inputs[('color_aug', -1, 0)][:, cam, ...] for cam in range(self.num_cams)], 1)
            sf_images_next = jt.stack([inputs[('color_aug', 1, 0)][:, cam, ...] for cam in range(self.num_cams)], 1)
        ...
        if self.novel_view_mode == 'MF':
            packed_feats_last_list = packed_feats_last[lev:lev+1] \
                            + [nn.interpolate(feat, [up_h, up_w], mode='bilinear', align_corners=True) for feat in packed_feats_last[lev+1:]]
            packed_feats_next_list = packed_feats_next[lev:lev+1] \
                            + [nn.interpolate(feat, [up_h, up_w], mode='bilinear', align_corners=True) for feat in packed_feats_next[lev+1:]]

        packed_feats_agg = self.conv1x1(jt.contrib.concat(packed_feats_list, dim=1))
        if self.novel_view_mode == 'MF':
            packed_feats_agg_last = self.conv1x1(jt.contrib.concat(packed_feats_last_list, dim=1))
            packed_feats_agg_next = self.conv1x1(jt.contrib.concat(packed_feats_next_list, dim=1))
...
class DepthDecoder(nn.Module):
    ...
    def execute(self, input_features):
        outputs = {}
